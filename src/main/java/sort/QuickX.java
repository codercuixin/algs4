...
public class QuickX {
    ...
    private static int partition(Comparable[] a, int lo, int hi){
        ...
        //a[lo[ is the unique largest element
        while(less(a[++i], v)){
            if(i == hi){
                exch(a, lo, hi);
                return hi;
        ...
        //a[lo] is the unique smallest element
        while(less(v, a[--j])){
            if(j==lo+1){
                return lo;
            }
    ...
    private static int median3(Comparable[] a, int i , int j , int k){
        if(less(a, i,j,k) || less(a,k,j, i) ){
            return j;
        }else if(less(a, j,i,k) || less(a, k,i,j)){
            return i;
        }else if(less(a, i,k,j) || less(a,j, k,i)){
            return k;
        }
        return j;
    }
    private static boolean less(Comparable[] a, int i, int j, int k){
        if(less(a[i], a[j]) && less(a[j], a[k])){
            return true;
        }
        return false;
    }
